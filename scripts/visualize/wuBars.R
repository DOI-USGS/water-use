visualize.wuBars <- function(viz=getContentInfo('wuBars'), stateUse=readData('calc-waterData'), 
                             natlUse=readData('calc-nationalData')$nationalData, outfile=viz[["location"]]) {
  ...
  waterTots <- stateUse %>%
    ...
    filter(category %in% c('Industrial','Irrigation','Public Supply','Thermoelectric'))
